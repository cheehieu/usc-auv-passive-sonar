...
void outputFile(uint8_t data_array[], int length, int adc_id)
{
        std::ofstream file ("1_adc_samples.txt");
        if ( file.is_open() )
        {
                short int input, valid_data_point;
                for (int i=0 ; i<length ; i+=2)
                {
                        input = (data_array[i] << 8) + data_array[i+1];         //concatenate bytes
                        valid_data_point = (input & 0x3ffc) >> 2;               //shift for correct bits
                        if(valid_data_point >= 0x0800)  valid_data_point = valid_data_point - 0x1000;   //convert 2's complement for plot
                        file << valid_data_point << ",";
                }
                file.close();
        }
